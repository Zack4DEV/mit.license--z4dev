...
export default async function getRoute(request, response) {
  ...
  try {
    ...
    response
      .status(500)
      .send(`An internal error occurred - open an issue on https://github.com/Zeh4DEV/mit-license with the following information: ${message}`)
    return
  }
  ...
  if (user.copyright) {
    if (is.string(user.copyright)) {
      ...
      name = user.copyright
        .map((value) => (isPlainText ? value : htmlEscape(value)))
        .join(', ')
    } else {
  ...
  try {
    const content = await renderFile(
      path.join(directoryName, '..', 'licenses', `${license}.ejs`),
      {
        info: `${year} ${name}`,
